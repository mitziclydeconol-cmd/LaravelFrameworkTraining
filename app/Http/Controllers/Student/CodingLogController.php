...
class CodingLogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('language'))   $query->where('programming_language', $request->language);
    ...
    public function store(StoreCodingLogRequest $request)
    {
        ...
        $newBadges = $this->badgeService->checkAndAward(auth()->user()->fresh('badges', 'codingLogs', 'aiFeedbackLogs', 'codingGoals'));

        $message = 'Coding log saved! Keep it up! 🎉';
        if (count($newBadges) > 0) {
            $names   = collect($newBadges)->pluck('name')->join(', ');
            $message .= " You earned: {$names}! 🏆";
        }

        return redirect()->route('student.logs.show', $log)->with('success', $message);
    }

    public function show(CodingLog $log)
    {
        $this->authorizeLog($log);
        $log->load(['subject', 'aiFeedbackLogs' => fn ($q) => $q->latest()->take(3), 'comments.user', 'selfAssessment']);
    ...
    public function update(StoreCodingLogRequest $request, CodingLog $log)
    {
        ...
        return redirect()->route('student.logs.show', $log)->with('success', 'Log updated!');
    }
    ...
    public function destroy(CodingLog $log)
    {
        $this->authorizeLog($log);
        $log->delete();
        return redirect()->route('student.logs.index')->with('success', 'Log deleted.');
    }
    ...
    public function getAiFeedback(CodingLog $log)
    {
        ...
        // Check AI explorer badge
        $this->badgeService->checkAndAward(auth()->user()->fresh('badges', 'codingLogs', 'aiFeedbackLogs', 'codingGoals'));

        if ($result['success']) return back()->with('ai_feedback', $result['feedback']);
    ...
    {
        return ['PHP','JavaScript','TypeScript','Python','Java','C','C++','C#','Ruby','Go','Rust','Swift','Kotlin','SQL','HTML/CSS','Bash','R','MATLAB','Other'];
    }
}
